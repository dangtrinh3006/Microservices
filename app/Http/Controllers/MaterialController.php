...
class MaterialController extends Controller
{
    ...
    public function update($id, Request $req)
    {
        ...
        if ($newNameMal == $oldNameMal) {
            ...
            $nameMal = $newNameMal;
            if ($this->checkNameMaterial($nameMal)) {
                session()->put('error_nameexists', true);
                return redirect()->back();
            }
        }
        $slug = Str::slug($nameMal);
        $nglieu->slug = $slug;
        $nglieu->ten_nglieu = $nameMal;
        $nglieu->gia_nhap = $req->gia_nhap;
        $nglieu->so_luong = $req->so_luong;

        $img = $req->file('MaterialImage');
        if ($img) {
            $newImage = rand(1, 9999999) . '.' . $img->getClientOriginalExtension();
            $img->move('uploads/materials', $newImage);
            $nglieu->hinh_anh = $newImage;
        }
    ...
    public function checkNameMaterial($name)
    {
        $mal = Materials::where('ten_nglieu', $name)->get('ten_nglieu');
        $checkName = "";
        foreach ($mal as $m) {
            $checkName = $m->ten_nglieu;
        }
        if ($name == $checkName) {
            return true;
        }
