...
//Dashboard
Route::get('admin/dashboard', 'DashboardController@show')->name('showDashboard')->middleware('auth');

Route::group(['middleware' => ['auth', 'checkrole', 'role2','role3','role4']], function () {
    ...
    Route::get('admin/doi-mat-khau', 'DashboardController@changepasswview')->name('viewupdatepass');
...
//category
Route::group(['middleware' => ['auth', 'role3']], function () {
...
Route::group(['middleware' => ['auth', 'role4']], function () {
...
//products
Route::group(['middleware' => ['role3', 'auth']], function () {
...
Route::get('/admin/logout', 'LoginController@logout')->name('auth.logout');

//nguyen lieu
Route::group(['middleware' => ['role4', 'auth']], function () {
...
Route::group(['middleware' => ['role2', 'auth']], function () {
...
Route::group(['middleware' => ['auth','role6']], function () {
...
Route::group(['middleware' => ['auth','role2']], function () {
    //xử lí đơn hàng
...
// thêm phí vận chuyển
Route::group(['middleware' => ['auth', 'role5']], function () {
...
Route::group(['namespace' => 'frontend'], function () {
    ...
    // tra cứu đơn hàng
    Route::get('tra-cuu-don-hang', 'HomeController@searchOrder')->name('search.order');
    ...
    //tin tức
    Route::get('tin-tuc',  'PostsController@index')->name('get.posts');
    Route::get('tin-tuc/{slug}',  'PostsController@getPosts')->name('detail.posts');
    Route::get('chinh-sach/{slug}',  'PostsController@showPolicy')->name('show.policy');


    Route::get('cua-hang',  'AboutController@index')->name('about');
    Route::get('lien-he',  'AboutController@contactIndex')->name('index.contact');
    Route::post('lien-he',  'AboutController@contact')->name('send.contact');


    Route::get('thuc-don',  'ProductController@index')->name('product');
    Route::get('thuc-don/{p}',  'ProductController@detail')->name('detail');
    Route::post('search',  'ProductController@search')->name('get.search');


    //tài khoản

    Route::get('tai-khoan/{nav}', 'LoginSocialController@getInfo')->name('get.infouser');
    ...
    // tat ca khuyen mai
    Route::get('khuyen-mai', 'CouponController@getAllPromotion')->name('get.all.promotion');
